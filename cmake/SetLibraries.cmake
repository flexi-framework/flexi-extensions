...
IF (OpenMP_FOUND)
  ...
  MESSAGE (STATUS "ATTENTION: [OpenMP] found in system libraries, BUT WILL BE INGORED! Building WITHOUT OpenMP instead!")
...
IF(LIBS_USE_OPENMP)
  MESSAGE (WARNING "[OpenMP] not supported in this version of FLEXI. Build WITHOUT OpenMP instead!")
...
IF(LIBS_USE_FFTW)
  ...
  FIND_LIBRARY(FFTW_LIBRARY NAMES fftw3 HINTS $ENV{FFTW_DIR}/lib     ${LIBS_EXTERNAL_LIB_DIR}/fftw/build/lib    )
  IF (LIBS_USE_OPENMP)
    FIND_LIBRARY(FFTW_OMP_LIBRARY NAMES fftw3_omp HINTS $ENV{FFTW_DIR}/lib ${LIBS_EXTERNAL_LIB_DIR}/fftw/build/lib)
  ENDIF()
  IF(FFTW_INCLUDES STREQUAL "FFTW_INCLUDES-NOTFOUND" OR  FFTW_LIBRARY     STREQUAL "FFTW_LIBRARY-NOTFOUND"
    ...
    SET(FFTW_FLAGS tar xzf ${POSTI_FFTWTAG}.tar.gz && FC=${CMAKE_Fortran_COMPILER} CC=${CMAKE_C_COMPILER} F77=${CMAKE_F77_COMPILER} ${POSTI_FFTWDIR}/src/fftw/${POSTI_FFTWTAG}/configure --prefix=${POSTI_FFTWDIR}/build --enable-shared)
    IF (LIBS_USE_OPENMP)
    ...
    SET(FFTW_OMP_LIBRARY ${POSTI_FFTWDIR}/build/lib/libfftw3_omp.a)
  ...
  # Append the FFTW paths to the linking and include paths
...
IF(LIBS_USE_SMARTREDIS)
  ADD_DEFINITIONS(-DUSE_SMARTREDIS=1)
  ...
  # We first try to find SmartRedis, but if we fail, we build it from source anyways
  IF (NOT LIBS_BUILD_SMARTREDIS)
    FIND_LIBRARY(LIBS_SMARTREDIS_LIB     smartredis         HINTS $ENV{SMARTREDIS_DIR}/install/lib     ${LIBS_EXTERNAL_LIB_DIR}/SmartRedis/src/smartredis/build/Release   )
    FIND_LIBRARY(LIBS_SMARTREDIS_FOR_LIB smartredis-fortran HINTS $ENV{SMARTREDIS_DIR}/install/lib     ${LIBS_EXTERNAL_LIB_DIR}/SmartRedis/src/smartredis/build/Release   )
    ...
    IF(    LIBS_SMARTREDIS_LIB     STREQUAL "LIBS_SMARTREDIS_LIB-NOTFOUND"
	OR LIBS_SMARTREDIS_FOR_LIB STREQUAL "LIBS_SMARTREDIS_FOR_LIB-NOTFOUND"
	OR LIBS_SMARTREDIS_INCLUDE STREQUAL "LIBS_SMARTREDIS_INCLUDE-NOTFOUND")
      MESSAGE(STATUS "SmartRedis NOT FOUND on path! Make sure to export root directory as 'SMARTREDIS_DIR'!")
      SET(LIBS_BUILD_SMARTREDIS ON)
    ELSE()
      MESSAGE(STATUS "Found SmartRedis library at ${LIBS_SMARTREDIS_LIB}.")
      MESSAGE(STATUS "Found SmartRedis Fortran library at ${LIBS_SMARTREDIS_FOR_LIB}.")
      MESSAGE(STATUS "Found SmartRedis include at ${LIBS_SMARTREDIS_INCLUDE}.")
    ENDIF()
  ENDIF()

  # Build from source if requested by user or if not found.
  IF (LIBS_BUILD_SMARTREDIS)
    SET (LIBS_SMARTREDIS_DOWNLOAD https://github.com/CrayLabs/SmartRedis.git )
    SET(LIBS_SMARTREDIS_TAG "v0.5.2" CACHE STRING "SmartRedis version tag")
    SET(LIBS_SMARTREDIS_DIR ${LIBS_EXTERNAL_LIB_DIR}/SmartRedis)
    # Check if already built
    IF (NOT EXISTS "${LIBS_SMARTREDIS_DIR}/src/smartredis/install/lib/libsmartredis-fortran.so")
      ...
      ExternalProject_Add(smartredis
        ...
        UPDATE_COMMAND ""
        CONFIGURE_COMMAND make -j1 lib-with-fortran # Explicitly run in single! SmartRedis sets #Procs explicitly in its Makefile and will fail with -jX option!
        BUILD_IN_SOURCE 1
      )
      LIST(APPEND SELFBUILTEXTERNALS smartredis)
    ENDIF()
    SET(LIBS_SMARTREDIS_INCLUDE "${LIBS_SMARTREDIS_DIR}/src/smartredis/install/include/")
    SET(LIBS_SMARTREDIS_LIB     "${LIBS_SMARTREDIS_DIR}/src/smartredis/build/Release/libsmartredis.so")
    SET(LIBS_SMARTREDIS_FOR_LIB "${LIBS_SMARTREDIS_DIR}/src/smartredis/build/Release/libsmartredis-fortran.so")
    MESSAGE(STATUS "SmartRedis library set to ${LIBS_SMARTREDIS_LIB}.")
    MESSAGE(STATUS "SmartRedis Fortran library set to ${LIBS_SMARTREDIS_FOR_LIB}.")
    MESSAGE(STATUS "SmartRedis include set to ${LIBS_SMARTREDIS_INCLUDE}.")
  ENDIF()
  LIST(APPEND linkedlibs ${LIBS_SMARTREDIS_LIB} ${LIBS_SMARTREDIS_FOR_LIB})
  INCLUDE_DIRECTORIES(${LIBS_SMARTREDIS_INCLUDE})
ELSE()
  ADD_DEFINITIONS(-DUSE_SMARTREDIS=0)
  MESSAGE(STATUS "Building Flexi WITHOUT SmartRedis")
ENDIF()
