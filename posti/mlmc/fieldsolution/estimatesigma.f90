...
SWRITE(UNIT_stdOut,'(1("**     START SIGMA ESTIMATE     **"))')
SWRITE(UNIT_stdOut,'(1("**********************************"))')
SWRITE(UNIT_stdOut,'(132("="))')
...
nEnd=nEnd+nPrevious
!CALL ReadAttribute(File_ID,'nIter',1,IntScalar=nIter)
CALL CloseDataFile()
...
SWRITE(UNIT_stdOut,'(132("="))')
! Bias:
BiasField = snSamples*(UFineSum-UCoarseSum)
Bias = 0.
CALL BiasL2(BiasField(VarAna,:,:,:,:),Bias)
